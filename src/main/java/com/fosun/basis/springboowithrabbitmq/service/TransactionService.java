...
import com.fosun.basis.springboowithrabbitmq.utils.RabbitMqConnFactoryUtils;
 ...
 * autoAck=false手动应对的时候是支持事务的，也就是说即使你已经手动确认了消息已经收到了，
 * 但在确认消息会等事务的返回解决之后，在做决定是确认消息还是重新放回队列，如果你手动确认现在之后，
...
public class TransactionService {
    ...
    public void publish()
        ...
        try {
            ...
            log.info("事务模式，发送10条数据，执行花费时间：" + (endTime - startTime) + "s");
    ...
    public void consume() throws IOException, TimeoutException, InterruptedException {
        ...
        Connection conn = RabbitMqConnFactoryUtils.getRabbitConn();
        Channel channel = conn.createChannel();
        channel.queueDeclare(TX_QUEUE, true, false, false, null);
        // 声明事务
        channel.txSelect();
